...
class DevedorRepository:
    @staticmethod
    def get_all(page=1, per_page=20, search=None, sort='nome_razao_social', order='asc'):
        query = Devedor.query.filter_by(ativo=True)
    ...
    @staticmethod
    def get_by_id(id):
        return Devedor.query.filter_by(id=id, ativo=True).first()

    @staticmethod
    def get_by_cpf_cnpj(cpf_cnpj):
        return Devedor.query.filter_by(cpf_cnpj=cpf_cnpj, ativo=True).first()
